...
namespace Catharsis.Commons.Domain
{
  ...
  public sealed class DownloadsCategoryTests : CategoryUnitTests<DownloadsCategory>
  {
    ...
    public void Xml_Method()
    {
      ...
      Assert.True(new DownloadsCategory("language", "name") { Id = 1 }.Xml().ToString() == xml.ToString());
      ...
      Assert.True(new DownloadsCategory("language", "name", new DownloadsCategory("parent.language", "parent.name") { Id = 2 }, "description") { Id = 1 }.Xml().ToString() == xml.ToString());
      Assert.True(DownloadsCategory.Xml(category.Xml()).Equals(category));
    }
  }
}