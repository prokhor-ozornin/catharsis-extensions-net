...
namespace Catharsis.Commons.Domain
{
  ...
  public sealed class AudiosCategoryTests : CategoryUnitTests<AudiosCategory>
  {
    ...
    public void Xml_Method()
    {
      ...
      Assert.True(new AudiosCategory("id", "language", "name").Xml().ToString() == xml.ToString());
      ...
      Assert.True(new AudiosCategory("id", "language", "name", new AudiosCategory("parent.id", "parent.language", "parent.name"), "description").Xml().ToString() == xml.ToString());
      Assert.True(AudiosCategory.Xml(category.Xml()).Equals(category));
    }
  }
}