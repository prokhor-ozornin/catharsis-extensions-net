...
namespace Catharsis.Commons.Domain
{
  ...
  public sealed class PlaycastTests : EntityUnitTests<Playcast>
  {
    ...
    ///   <seealso cref="Playcast(long, string, string, string, PlaycastsCategory, Audio, Image)"/>
    /// </summary>
    [Fact]
    public void Constructors()
    {
      ...
      Assert.Throws<ArgumentNullException>(() => new Playcast(1, "language", null, "text", new PlaycastsCategory()));
      Assert.Throws<ArgumentNullException>(() => new Playcast(1, "language", "name", null, new PlaycastsCategory()));
      Assert.Throws<ArgumentException>(() => new Playcast(1, "language", string.Empty, "text", new PlaycastsCategory()));
      Assert.Throws<ArgumentException>(() => new Playcast(1, "language", "name", string.Empty, new PlaycastsCategory()));
      playcast = new Playcast(1, "language", "name", "text", new PlaycastsCategory(), new Audio(), new Image());
      Assert.True(playcast.Id == 0);
      Assert.True(playcast.AuthorId == 1);
      Assert.True(playcast.DateCreated <= DateTime.UtcNow);
    ...
    public void Xml_Methods()
    {
      ...
      Assert.True(playcast.Audio == null);
      Assert.True(playcast.AuthorId == 2);
      Assert.True(playcast.Category == null);
      ...
      Assert.True(playcast.Text == "text");
      Assert.True(new Playcast(2, "language", "name", "text") { Id = 1, DateCreated = DateTime.MinValue, LastUpdated = DateTime.MaxValue }.Xml().ToString() == xml.ToString());
      ...
      Assert.True(playcast.Tags.Count == 0);
      Assert.True(playcast.Text == "text");
      Assert.True(new Playcast(2, "language", "name", "text", new PlaycastsCategory("category.language", "category.name") { Id = 5 }, new Audio(new File("audio.file.contentType", "audio.file.name", "audio.file.originalName", Guid.Empty.ToByteArray()) { Id = 4, DateCreated = DateTime.MinValue, LastUpdated = DateTime.MaxValue }, 1, 2) { Id = 3 }, new Image(new File("image.file.contentType", "image.file.name", "image.file.originalName", Guid.Empty.ToByteArray()) { Id = 7, DateCreated = DateTime.MinValue, LastUpdated = DateTime.MaxValue }, 1, 2) { Id = 6 }) { Id = 1, DateCreated = DateTime.MinValue, LastUpdated = DateTime.MaxValue }.Xml().ToString() == xml.ToString());
      Assert.True(Playcast.Xml(playcast.Xml()).Equals(playcast));
    }
  }
}