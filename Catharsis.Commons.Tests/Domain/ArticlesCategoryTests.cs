...
namespace Catharsis.Commons.Domain
{
  ...
  public sealed class ArticlesCategoryTests : CategoryUnitTests<ArticlesCategory>
  {
    ...
    public void Xml_Method()
    {
      ...
      Assert.True(new ArticlesCategory("language", "name") { Id = 1 }.Xml().ToString() == xml.ToString());
      ...
      Assert.True(new ArticlesCategory("language", "name", new ArticlesCategory("parent.language", "parent.name") { Id = 2 }, "description") { Id = 1 }.Xml().ToString() == xml.ToString());
      Assert.True(ArticlesCategory.Xml(category.Xml()).Equals(category));
    }
  }
}