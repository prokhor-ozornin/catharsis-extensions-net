...
public static class StringExtensions
{
  ...
  public static byte[] Bytes(this string text, Encoding? encoding = null) => text.Length > 0 ? (encoding ?? Encoding.Default).GetBytes(text) : Array.Empty<byte>();
  ...
  public static byte[] Base64(this string text) => text.Length > 0 ? System.Convert.FromBase64String(text) : Array.Empty<byte>();
  ...
  public static byte[] Hex(this string text) => text.Length > 0 ? System.Convert.FromHexString(text) : Array.Empty<byte>();
#endif
  ...
  /// <returns></returns>
  public static string Reverse(this string text) => text.Length > 0 ? text.Reverse<char>().ToArray().Text() : string.Empty;
  ...
  /// <returns></returns>
  public static string Replace(this string text, params (string Name, object? Value)[] replacements)
  {
    if (text.Length <= 0)
    {
      return text;
    ...
    replacements.ForEach(replacement => result.Replace(replacement.Name, replacement.Value?.ToStringInvariant() ?? string.Empty));
  ...
  /// <param name="replacements"></param>
  /// <returns></returns>
  public static string Replace(this string text, object replacements) => text.Length > 0 ? text.Replace(replacements.Properties().AsArray()) : string.Empty;
  ...
  public static string Discard(this string text, int count) => text.Length <= 0 || count > text.Length ? string.Empty : count > 0 ? text.Remove(0, count) : text;
  ...
  /// <returns></returns>
  public static string DiscardLast(this string text, int count) => text.Length <= 0 || count > text.Length ? string.Empty : count > 0 ? text.Remove(text.Length - count, count) : text;
  ...
  public static string Unindent(this string text, char value)
  {
    ...
    {
      result.AppendLine(line.SkipWhile(character => character == value).ToArray().Text());
    }
  ...
  /// <returns></returns>
  public static Process Execute(this string command, params string[] arguments)
  {
    var process = command.ToProcess();

    process.StartInfo.ArgumentList.AddAll(arguments);
