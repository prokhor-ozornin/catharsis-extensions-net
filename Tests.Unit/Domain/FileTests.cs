...
namespace Catharsis.Commons.Domain
{
  ...
  public sealed class FileTests : EntityUnitTests<File>
  {
    ...
    /// <summary>
    ///   <para>Performs testing of <see cref="File.Hash"/> property.</para>
    /// </summary>
    [Fact]
    public void Hash_Property()
    {
      Assert.Throws<ArgumentNullException>(() => new File { Hash = null });
      Assert.Throws<ArgumentException>(() => new File { Hash = string.Empty });
      Assert.True(new File { Hash = "hash" }.Hash == "hash");
    }
    ...
    public void Constructors()
    {
      ...
      Assert.True(file.Data == data);
      Assert.True(file.DateCreated <= DateTime.UtcNow);
      Assert.True(file.Hash == data.EncodeSHA512().EncodeHex());
      Assert.True(file.LastUpdated <= DateTime.UtcNow);
      Assert.True(file.Name == "name");
      Assert.True(file.OriginalName == "originalName");
    ...
    public void Xml_Methods()
    {
      ...
      Assert.True(file.Hash == Guid.Empty.ToByteArray().EncodeSHA512().EncodeHex());
